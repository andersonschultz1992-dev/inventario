...
function kpiHtml(rows) {
  ...
  return cell(total, 'Hosts') + cell(ligados, 'Ligados') + cell(doms, 'Domínios')
    + cell(tecs, 'Tecnologias') + cell(eol, 'SO fora de suporte') + cell(warn, 'RHEL 7 (ELS)');
}
...
function tableHtml(rows) {
  ...
  const body = rows.map(r => `
    <tr>
      ...
      <td class="rp-mono">${escapeHtml(r.release ?? '—')}</td>
  ...
  return `<table class="rp-table">
    <thead><tr><th>Hostname</th><th>Time</th><th>Domínio</th><th>Tecnologias</th>
    <th>SO</th><th>Release</th><th>Java</th><th>Situação</th></tr></thead>
    <tbody>${body}</tbody></table>`;
}

const REPORT_CSS = `
  .rp-root { width: 1080px; background: #FFFFFF; color: #16241B; font-family: Inter, Arial, sans-serif; padding: 28px 32px; }
